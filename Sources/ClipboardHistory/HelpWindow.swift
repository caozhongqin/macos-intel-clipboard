...
class HelpWindow {
    ...
    func show() {
        ...
        appendSection("🚀 开机自启")
        appendText("如需开机自动启动本应用，请按以下步骤设置：")
        appendText("1. 打开「系统设置」→「用户与群组」→「登录项」")
        appendText("2. 在「当您登录时，这些项目将自动打开」列表中，点击 + 号")
        appendText("3. 在 Finder 中选择 Clipboard.app 添加即可")
        appendText("（应用位于项目目录下的 Clipboard.app）")
        appendSection("ℹ️ 关于")
